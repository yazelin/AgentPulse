...
Antigravity CLI, Codex CLI, GitHub Copilot CLI). Tauri v2 cross-platform fork of the
...
field, Copilot CLI takes parallel `bash` and `powershell` fields, Antigravity
runs hooks via `sh -c` / `cmd /c`, and Codex CLI disables hooks on Windows
entirely. Maintaining several quotation dialects of the same command is
...
- **Antigravity CLI (`agy`)**: `PreInvocation` → `UserPromptSubmit` (sets Working; `SessionStart` would set Idle and a tool-less turn's `Stop` would then emit no completion sound); `PostToolUse` / `Stop` pass through (already PascalCase). The event name isn't in agy's stdin payload — the sidecar injects it from its 2nd arg (see below).
...
completion sound. Any event that fires mid-turn (Copilot's `subagentStop`
on child-agent completion) must *not* map to `Stop`: the mid-turn
transition steals the completion sound, and the real end-of-turn signal
(`agentStop`) is then swallowed because state is already `Idle`. These
fall through to the state machine's no-op `_` arm so state stays
`Working`. (For agy, `PreInvocation` fires before each model call and
maps to `SessionStart` — deliberately not `Stop` — for the same reason.)

### Field aliasing (hook_event.rs)

Different CLIs use different field names for the same thing:
- `session_id` / `sessionId` / `session` / `conversationId` (agy) → normalized to `session_id`
...
- **Antigravity (`agy`)** — `~/.gemini/config/hooks.json`: a DIFFERENT model. Named hooks keyed `{name: {Event: [handlers]}}` (one extra nesting level vs Claude), only 5 events (`PreToolUse`/`PostToolUse`/`PreInvocation`/`PostInvocation`/`Stop`), and **synchronous** — each hook must print a JSON result on stdout, and the stdin payload has no event-name field (event = the config key). `install_antigravity_hooks` writes a single `"agentpulse"` named hook wiring only `PreInvocation`/`PostToolUse`/`Stop` (all satisfied by a bare `{}` on stdout); `PreToolUse` is skipped (needs a `decision`). The event name rides as the sidecar's 2nd arg (`agent-pulse-hook antigravity Stop`), which `inject_event` folds into the body as `hook_event_name` before POSTing, then the sidecar echoes `{}`. `remove_provider` special-cases antigravity by dropping the whole `"agentpulse"` root key.
...
**Note — sidecar 2nd arg is antigravity-only.** `hook_cmd(provider_id)`
builds `"path" <provider>` for every provider; `hook_cmd_ev(provider, event)`
adds the trailing event name and is used only by `install_antigravity_hooks`.
The sidecar prints `{}` to stdout ONLY when a 2nd arg is present, so
fire-and-forget providers (Claude et al., which may themselves interpret
hook stdout) stay silent. (A prior `hook_cmd_powershell()` helper existed
for Gemini's hard-coded PowerShell + `&` call operator; removed with Gemini.)
